...
public:
    ...
    using M3T = Eigen::TensorFixedSize<Scalar, Eigen::Sizes<PolyOrder + 1, PolyOrder + 1, PolyOrder + 1, PolyOrder + 1>>;
...
/** @brief : Compute M3 Tensor for Eva */
template<int PolyOrder,
         int NumSegments,
         int NX,
         int NU,
         int NP,
         typename Scalar>
void Legendre<PolyOrder, NumSegments, NX, NU, NP, Scalar>::compute_m3_tensor()
{
    /** naive implementation */
        ...
        for(int k = 0; k <= PolyOrder; ++k){
            for(int i = 0; i <= PolyOrder; ++i){
                for(int j = 0; j <= PolyOrder; ++j){
                    ...
                    {
                        // compute projection //
                        inner_prod += poly_eval(l, _Nodes[n]) * poly_eval(i, _Nodes[n]) * poly_eval(j, _Nodes[n]) * poly_eval(k, _Nodes[n]) * _QuadWeights[n];
                    }
                    if(l == PolyOrder)
                        _M3Tensor(i, j, k, l) = (PolyOrder / 2.0) * inner_prod;
                    else
                        _M3Tensor(i, j, k, l) = ((2.0 * k + 1) / 2.0) * inner_prod;
                    //std::cout << _Galerkin(i, j, k) << " " ;
                }
                //std::cout << "\n";
            }
